module dcsg(clk, we, data, sound_out);
input clk, we;
input [7:0] data;
...
always @(posedge clk) begin
	if (update_period) begin
		update_period <= 1'b0;
		period[last[2:1]] <= { freq_u[last[2:1]], freq_l[last[2:1]] };
	end
	else if (we & ~&regsel[2:1] & ~regsel[0])
		update_period <= 1'b1;
	//
	if (~update_period & update_period3) begin
		update_period3 <= 1'b0;
		period[2'b11] <= &nf ? { freq_u[2'b10], freq_l[2'b10], 1'b0 } : 11'h20 << nf;
	end
	else if (we & (regsel[2] & (regsel[1] | &nf) & ~regsel[0]))
		update_period3 <= 1'b1;
end
...
always @(posedge clk)
	if (|divcnt) divcnt <= divcnt - 1'b1;
		...
		if (we & regsel == 3'b110) lfsr <= 15'b1;
		else if (trig & &i)
